...
export default function PeopleDetails({
  ...
  const fetchUser = async () => {
    ...
    setUser(user);
    setName(`${user.firstName} ${user.lastName}`); // Set initial name state
  };
      ...
      <div className="text-danger fs-4">
        {!editing && (
          <FaPencilAlt
            ...
            className="float-end fs-1 mt-2 text-danger"
          />
        )}
        {editing && (
          <FaCheck
            onClick={() => saveUser()}
            className="float-end fs-1 text-success mt-2 me-2"
          />
        )}
        {!editing && (
          <div className="text-danger" onClick={() => setEditing(true)}>
